...
Architecture overview and contributor workflow for **naeasy**.
...
cd src-tauri && cargo test      # unit tests (domain + service)
npm run dev                     # hot-reload dev app
./dev-install.sh                # build from source → /Applications (incremental)
./scripts/build-linux.sh        # Linux .deb via Docker (amd64 + arm64)
```

`./install.sh` in the repo root is the END-USER installer (downloads a
prebuilt release) — for development use `dev-install.sh`.

## Release

```bash
./release.sh 1.2.0              # bump versions, commit, tag
git push --follow-tags          # CI builds macOS (arm64/x64) + Linux debs
